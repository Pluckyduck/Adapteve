...
{
    public class WinsockHook : IDisposable
    {
        ...
        private IntPtr Recieve(IntPtr socket, int len)
        {
            ...
            if (result == -1)
            {
                Utility.Log("Error2: " + WSAGetLastError());
                return IntPtr.Zero;
            }
        ...
        private IntPtr RecieveAuth(IntPtr socket, int len)
        {
            ...
            if (result == -1)
            {
                Utility.Log("Error3: " + WSAGetLastError());
                return IntPtr.Zero; ;
            }
            ...
            if (result != 2)
            {
                Utility.Log("Proxy: Bad response from server");
                return IntPtr.Zero;
            }
            return buffer;
        }
        ...
        private IntPtr SetUpAuthenticateRequest(string username, string password, out int index)
        {
            ...
            {
                rawBytes = Encoding.Default.GetBytes(username);   //USERNAME
                for (int i = 0; i < rawBytes.Length; i++)
                {
                    Marshal.WriteByte(authenticateBuffer + index++, rawBytes[i]);
                }
            }
            Marshal.WriteByte(authenticateBuffer + index++, Convert.ToByte(password.Length));  //PASSWORD
            if (password.Length > 0)
            {
                rawBytes = Encoding.Default.GetBytes(password);  //PASSWORD
                for (int i = 0; i < rawBytes.Length; i++)
                {
                    Marshal.WriteByte(authenticateBuffer + index++, rawBytes[i]);
                }
        ...
        private bool VerifyBindResponse(IntPtr buffer)
        {
            ...
            if (recvBytes[1] != 0)
            {
                ...
                if (recvBytes[1] == 5)
                    Utility.Log("connection refused by destination host");
                if (recvBytes[1] == 6)
                    Utility.Log("TTL expired");
                if (recvBytes[1] == 7)
                    Utility.Log("command not supported / protocol error");
                if (recvBytes[1] == 8)
                    Utility.Log("address type not supported");

                Utility.Log("Proxy: Connection error binding eve server");
                return false;
            }
            return true;
        }
